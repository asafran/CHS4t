...
{
    QString  secName = "SM", id, ba;
    if(cfg.getString(secName, "ID_REVERS", id) && cfg.getString(secName, "BA_REVERS", ba))
        reverser_sw = MultiSwitch(id, ba);

    if(cfg.getString(secName, "ID_MAIN", id) && cfg.getString(secName, "BA_MAIN", ba))
        kr21_sw = MultiSwitch(id, ba);

    if(cfg.getString(secName, "ID_OP", id) && cfg.getString(secName, "BA_OP", ba))
        kr21_op = MultiSwitch(id, ba);
     kr21_sw.setPos(N);
     reverser_sw.setPos(1);
    if(cfg.getString("LATCH", "ID", id))
        latch.setKey(id);
    latch.attach();
...
void Km21KR2::stepKeysControl(double t, double dt)
{
    ...
    if(latch.lock())
    {
        bool *data = (bool*)latch.data();
        if ((!reverseIsPressedOneTime && (TO_INT(mainShaftPos) == 0) && (TO_INT(fieldWeakShaft) == 0)) && *data)
        ...
        if(reverseState != prev_reverseState)
        {
            reverser_sw.setPos(reverseState + 1);
            prev_reverseState = reverseState;
        }
        latch.unlock();
    }
    if (reverseState == 0)
    ...
    {
        if (!is_inc && fieldWeakShaft == 0 && is_dec)
        {
            emit soundPlay("21KR_+_0");
            kr21_sw.setPos(N);
        }
